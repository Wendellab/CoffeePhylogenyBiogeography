...
trfn = "Coffea.nwk"
tr = read.tree(trfn)
# drop outgroups and duplicate sample
tr <- drop.tip(tr, "C.charrieriana-2")
tr <- drop.tip(tr, "Bertiera")
tr <- drop.tip(tr, "Calycosiphonia")
tr <- drop.tip(tr, "Argocoffeopsis")
tr <- drop.tip(tr, "Belonophora")
tr <- drop.tip(tr, "Tricalysia")

# find the node that corresponds to the Coffea/Psilanthus divergence
plot(tr, cex = 0.5)
nodelabels()

# add dates to the tree
chron.tre <- chronopl(tr, 1, age.min = 5.3, age.max = 18.61, node = 85, S = 28800, CV=TRUE)
...
geogfn = "Coffea_geog.no_outgroup.data"
...
BioGeoBEARS_run_object$dispersal_multipliers_fn = "manual_dispersal_multipliers_mod.txt"
...
runslow = TRUE
resfn = "Coffea.final.DEC.forpub.RData"
if (runslow)
    {
...
resfn = "Coffea.final.DECJ.forpub.RData"
runslow = TRUE
if (runslow)
    {
...
pdffn = "Coffea.chron.DECJ.forpub.pdf"
pdf(pdffn, width=15, height=30)

analysis_titletxt ="BioGeoBEARS DEC+J on Coffea with dispersal modifiers"
...
# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.45, tipcex=0.7, statecex=0.7, splitcex=0.6, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges, plotlegend=TRUE)

dev.off()

pdffn = "Coffea.chron.DECJ.forpub.expanded.pdf"
...
svg("Coffea.R.svg", width = 6, height = 12)
...
runslow = TRUE
resfn = "Coffea.final.DIVA.forpub.RData"
if (runslow)
    {
    res = bears_optim_run(BioGeoBEARS_run_object)
    res    
...
resfn = "Coffea.final.DIVAJ.forpub.RData"
runslow = TRUE
if (runslow)
    {
...
runslow = TRUE
resfn = "Coffea.fina.BAY.forpub.RData"
if (runslow)
    {
    res = bears_optim_run(BioGeoBEARS_run_object)
    res    
...
resfn = "Coffea.final.BAYJ.forpub.RData"
runslow = TRUE
if (runslow)
    {
    res = bears_optim_run(BioGeoBEARS_run_object)
...
# Loads to "restable"
save(restable, file="restable_v1.forpub.RData")
load(file="restable_v1.forpub.RData")

# Loads to "teststable"
save(teststable, file="teststable_v1.forpub.RData")
load(file="teststable_v1.forpub.RData")
